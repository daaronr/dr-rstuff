---
title: "marking-beem101-final-2020"
...
student_responses <- "Copy of BEEM101 answers.xlsx" #mcq
adhoc_adjust <- 0

longansfile <- "101_final_20.csv"

outfile <-  "beem101_final_results_2020.csv"
...
(markpct_p80 <-  sr %>% select(starts_with("markpct_")) %>%
...
sr <- sr %>%
  mutate(
    total = rowSums(.[grep("mk_q*", names(.))], na.rm = TRUE),
    total_adjust = rowSums(.[grep("mk_q*", names(.))], na.rm = TRUE)+ adhoc_adjust
  )
```
...
sr_all <- sr %>% mutate(Cand_num = as.numeric(`Student Candidate Number`)) %>%
  ...
  mutate(total_mark= b_total + c_total + total_adjust) %>%
  select(Cand_num, matches("total_adjust|mk_max|Mark|total_mark|b_total|c_total"),starts_with("q"),-starts_with("qu")) %>%
  select(Cand_num,total_mark,b_total,starts_with("total"), everything()) %T>%
  print()
...
sr_all %>% select(starts_with("markpct_questions")) %>%
  dplyr::summarise_all(mean, na.rm=TRUE) %>% transpose()
