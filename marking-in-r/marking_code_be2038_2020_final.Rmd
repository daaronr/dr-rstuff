...
sr_all <- sr %>% mutate(cand_num = as.numeric(`Student Candidate Number`)) %>%
  ...
  select(cand_num, matches("cand_num|Choices|B_total|total_mark|mk_max|Mark|total.for|cmt|comments|missing|mcq_marks"),starts_with("q"),-starts_with("qu"),-fields_unmarked) %>%
...
mod_sample <- sr_all %>%
  ...
  select(cand_num, mark_class, total_mark, mcq_marks, B_total, Choices, matches("cmt|Comments") )   
