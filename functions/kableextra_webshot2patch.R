          ...
          density, ...)
{
  ...
  if (tools::file_ext(file) %in% c("png", "jpg", "jpeg", "pdf")) {
    ...
    kableExtra:::save_HTML(html_result, file = file_temp_html, libdir = temp_dir,
    ...
    if (self_contained) {
      temp_dir <- sub(pattern = "^[\\\\/]{1,2}", replacement = "",
                      tempfile(pattern = "lib", tmpdir = "", fileext = ""))
      kableExtra:::save_HTML(html_result, file = file, libdir = temp_dir,
                self_contained = TRUE)
      self_contained(file, file)
      unlink(file.path(dirname(file), temp_dir), recursive = TRUE)
    }
    else {
      kableExtra:::save_HTML(html_result, file = file, self_contained = FALSE)
    }
  }
...
unlockBinding(getNamespace("kableExtra"), sym = "save_kable_html")
assign("save_kable_html", save_kable_htmlW2, envir = getNamespace("kableExtra"))
